...
head_radius = int( ((width+height)/2) / (7) )
...
def hit(head_radius,x, y, head_center):
...
def xUpdate(head_radius,x):
    _, ball.x_position, _, xvelocity, _ = hit(head_radius,ball.x_position,ball.y_position,head_center)#call function for if head hits ball
    ball.x_position = ball.x_position + xvelocity #reposition ball based on the hit
    return(ball.x_position,xvelocity)

safetybouncecount = 0
def yUpdate(head_radius,safetybouncecount, y):
    _, _, ball.y_position, _, yvelocity = hit(head_radius,ball.x_position,ball.y_position,head_center)
...
while(cap.isOpened()):
    ...
    if ret==True:
        ...
        cv2.circle(frame, (center_x,center_y), (int(head_radius)), (0,255,0), -1)#radius must be changed for different computers/different screens
        head_center = (center_x, center_y)
            
        #pdb.set_trace()
        Ball.x_position,xvelocity = xUpdate(head_radius,Ball.x_position)
        ...
        #pdb.set_trace()
        Ball.y_position, safetybouncecount, yvelocity = yUpdate(head_radius,safetybouncecount,Ball.y_position)
        ...
        if cv2.waitKey(100) & 0xFF == ord('l'): #leftkey with mirror effect
            center_x = center_x - 50
        if cv2.waitKey(100) & 0xFF == ord('j'): #rightkey with mirrow effect
            center_x = center_x + 50
        if cv2.waitKey(100) & 0xFF == ord('i'): #upkey
            center_y = center_y - 50
        if cv2.waitKey(100) & 0xFF == ord('k'): #downkey
            center_y = center_y + 50
