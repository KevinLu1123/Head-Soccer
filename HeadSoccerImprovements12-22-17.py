...
class Player():
    def __init__(self, name = "", mass = 140):
        ...
        self.head_center = (int(width/4),int(height/4))#affecting the headballdistance calculation
...
def xUpdate(P,xvelocity,yvelocity):
    if P != None:
        _, ball.x_position, _, xvelocity, _ = P.hitresult(ball,xvelocity,yvelocity)#call function for if head hits ball
...
def yUpdate(safetybouncecount,P,xvelocity,yvelocity):
    if P != None:
        _, _, ball.y_position, _, yvelocity = P.hitresult(ball,xvelocity,yvelocity)
    ygravity = 2
...
while(cap.isOpened()):
    ...
    if ret==True:
        ...
        if Player_1.hit(x,y):
            ...
            Ball.y_position, safetybouncecount, yvelocity = yUpdate(safetybouncecount,None,xvelocity,yvelocity)
